...
function calculTotal(menusTmp) {
  ...
  for (let indexMenu = 0; menusTmp[indexMenu]; indexMenu++) {
    for (
      let indexProduct = 0;
      menusTmp[indexMenu].products[indexProduct];
      indexProduct++
    ) {
      if (menusTmp[indexMenu].products[indexProduct].cpt > 0) {
        ...
        if (
          ...
          menusTmp[indexMenu].products[indexProduct].status
        ) {
          totalProduct =
            totalProduct +
            menusTmp[indexMenu].products[indexProduct].priceInt *
              menusTmp[indexMenu].products[indexProduct].cpt;
        } else if (
          menusTmp[indexMenu].products[indexProduct].status === false &&
          menusTmp[indexMenu].products[indexProduct].choice === false
        ) {
          totalProduct =
            totalProduct + menusTmp[indexMenu].products[indexProduct].priceInt;
          for (
            let indexIngredient = 0;
            menusTmp[indexMenu].products[indexProduct].ingredients[
              indexIngredient
            ];
            indexIngredient++
          ) {
            if (
              ...
              for (
                ...
                if (
                  menusTmp[indexMenu].products[indexProduct].ingredients[
                    indexIngredient
                  ].choice[indexChoice].status
                ) {
                  totalProduct =
                    totalProduct +
                    menusTmp[indexMenu].products[indexProduct].ingredients[
                      indexIngredient
                    ].choice[indexChoice].price;
                }
              }
            }
          }
        ...
        total =
          total + totalProduct * menusTmp[indexMenu].products[indexProduct].cpt;
      }
    }
  }
  return total;
...
function Menu(props) {
  ...
  const addToCart = (menuId, productId) => {
    if (menus[menuId].products[productId].choice === true) {
      ...
      let newMenu = [...menus];
      newMenu[menuId].products[productId].cpt =
        newMenu[menuId].products[productId].cpt + 1;
  ...
  const handleClickOpen = (menuId, productId) => {
    ...
    for (
      let index = 0;
      menus[menuId].products[productId].ingredients[index];
      index++
    ) {
      let ingredientTmp = {};
      if (
        menus[menuId].products[productId].ingredients[index].type !== "choice"
      ) {
        ingredientTmp = {
          statusChecked: menus[menuId].products[productId].ingredients[index].statusChecked,
          type: menus[menuId].products[productId].ingredients[index].type,
          ingredientName:
            menus[menuId].products[productId].ingredients[index].ingredientName,
          price: menus[menuId].products[productId].ingredients[index].price,
          validate:
            menus[menuId].products[productId].ingredients[index].validate,
        };
      } else {
        let choicesTmp = [];
        for (
          let indexChoice = 0;
          menus[menuId].products[productId].ingredients[index].choice[
            indexChoice
          ];
          indexChoice++
        ) {
        ...
        ingredientTmp = {
          ...
          validate:
            menus[menuId].products[productId].ingredients[index].validate,
          choiceName:
            menus[menuId].products[productId].ingredients[index].choiceName,
          choiceNbr:
            menus[menuId].products[productId].ingredients[index].choiceNbr,
          choice: choicesTmp,
        };
      }
  ...
  const checkBoxChange = (ingredientIndex, choiceIndex) => {
    ...
    productDialogTmp.ingredients[ingredientIndex].choice[choiceIndex].status =
      !productDialogTmp.ingredients[ingredientIndex].choice[choiceIndex].status;
    if (
      productDialogTmp.ingredients[ingredientIndex].choice[choiceIndex].status
    ) {
      productDialogTmp.priceF =
        productDialogTmp.priceF +
        productDialogTmp.ingredients[ingredientIndex].choice[choiceIndex].price;
    } else {
      productDialogTmp.priceF =
        productDialogTmp.priceF -
        productDialogTmp.ingredients[ingredientIndex].choice[choiceIndex].price;
    ...
    for (
      let index = 0;
      productDialogTmp.ingredients[ingredientIndex].choice[index];
      index++
    ) {
      if (
        productDialogTmp.ingredients[ingredientIndex].choice[index].status ===
        true
      ) {
  ...
  const addProduct = () => {
    ...
    if (productDialog.choice === false) {
      let newMenu = [...menus];
      newMenu[menusID].products[productId].cpt =
        newMenu[menusID].products[productId].cpt + 1;
      setCptProduct(cptProduct + 1);
      ...
      for (let index = 0; productDialog.ingredients[index]; index++) {
        if (
          productDialog.ingredients[index].statusChecked === true &&
          productDialog.ingredients[index].type === "choice" &&
          productDialog.ingredients[index].validate === false
        ) {
          validate = false;
        }
      }
      if (validate) {
        ...
        for (let index = 0; menus[menusID].products[index]; index++) {
          if (menus[menusID].products[index].id === productDialog.id) {
            if (
              JSON.stringify(menus[menusID].products[index].ingredients) ===
              JSON.stringify(productDialog.ingredients)
            ) {
              let newMenu = [...menus];
              newMenu[menusID].products[index].cpt =
                newMenu[menusID].products[index].cpt + 1;
  ...
  const update = () => {
    ...
    for (let index = 0; newMenu[menusID].products[index]; index++) {
      if (
        ...
        productDialogTmp.cpt =
          productDialogTmp.cpt + menus[menusID].products[index].cpt;
        if (productId > index) {
  ...
  const checkBoxChangeIngredient = (ingredientIndex) => {
    ...
    productTmp.ingredients[ingredientIndex].validate =
      !productTmp.ingredients[ingredientIndex].validate;
    if (productTmp.ingredients[ingredientIndex].validate) {
      productTmp.priceF =
        productTmp.priceF + productTmp.ingredients[ingredientIndex].price;
    } else {
      productTmp.priceF =
        productTmp.priceF - productTmp.ingredients[ingredientIndex].price;
    }
    ...
    <Box sx={{ width: "100%" }}>
      {loading &&
        menus.map((menu, index1) => (
          <Accordion
            key={menu.productCategoryName}
            expanded={expanded === menu.panel}
            onChange={handleChange(menu.panel)}
          >
            <AccordionSummary>
                      ...
                      <Box sx={{ flexGrow: 1 }}>
                        <Grid container spacing={0}>
                          <Grid item xs={1} sx={{ textAlign: "center" }}>
                            {product.cpt > 0 && (
                              <Typography>{product.cpt}x</Typography>
                            )}
                          </Grid>
                          <Grid
                            item
                          ...
                          >
                            {product.priceF}
                            <>
                              {product.priceF % 1 === 0 && <>.00 MAD</>}
                              {product.priceF % 1 !== 0 && <> MAD</>}
                            </>
                          </Grid>
                          <Grid item xs={1}>
                            {product.cpt > 0 && (
        ...
        >
          <Card sx={{ height: "100%" }}>
            ...
            <CardContent>
              ...
              >
                <Grid container spacing={0}>
                  ...
                  <Grid item xs={4} sx={{ textAlign: "right" }}>
                    <Typography gutterBottom variant="h5" component="div">
                      {productDialog.priceInt}
                      <>
                        {productDialog.priceInt % 1 === 0 && <>.00 MAD</>}
                        {productDialog.priceInt % 1 !== 0 && <> MAD</>}
                      </>
                    </Typography>
                  </Grid>
                </Grid>
              ...
              <Divider />
              {productDialog.ingredients &&
                productDialog.ingredients.map((ingredient, index1) => (
                  <div key={index1}>
                    {ingredient.statusChecked === false && ingredient.type === "none" && (
                      <Box sx={{ flexGrow: 1, alignItems: "center" }}>
                        <Grid container spacing={0}>
                          ...
                          <Grid item xs={2} sx={{ textAlign: "right" }}>
                            <Checkbox
                              checked={ingredient.validate}
                              onChange={() => {
                                checkBoxChangeIngredient(index1);
                              }}
                            />
                          </Grid>
                        </Grid>
                    ...
                    {ingredient.type === "choice" && (
                      <Box sx={{ flexGrow: 1, alignItems: "center" }}>
                        ...
                        {ingredient.choice &&
                          ingredient.choice.map((choice, index2) => (
                            <Box
                              key={index2}
            ...
            <CardActions sx={{ width: "auto" }}>
              ...
              >
                ...
                Ajouter pour {productDialog.priceF}{" "}
                <>
                  {productDialog.priceF % 1 === 0 && <>.00 MAD</>}
                  {productDialog.priceF % 1 !== 0 && <> MAD</>}
                </>
              </Button>
      ...
      {cpt !== 0 && (
        <Link to={"/commande"}>
          <Fab
            aria-label="CheckOut"
            color="primary"
            sx={{ position: "fixed", bottom: "3%", right: "2%", zIndex: 1 }}
            onClick={() => {
              commandeClick();
            }}
          >
